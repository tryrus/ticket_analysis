...
from get_stations import get_station, write, read, is_stations, get_selling_time
...
class Main(QMainWindow, Ui_MainWindow):
    ...
    def query_time_click(self):
        ...
        if station in stations_time:  # 判断要搜索的站名是否存在
            name_lit, time_list = query_time(station, stations.get(station))  # 查询起售车站对应的站名与起售时间
            if len(name_lit) > 0:
